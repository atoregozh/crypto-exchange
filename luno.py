...
def calc_luno_withdrawals(request_map):
    """
        :param request_map: A dict which maps strings to dict objects of format
        {
            currency_pair: {
                from_symbol: string,
                'from_name': string,
    ...
    for key, val in request_map.items():
        ...
        luno_to_binance_currency_pairs.update({luno_pair : binance_pair})
    ...
    for ticker in luno_tickers['tickers']:
        ...
        trading_fee_response = luno_client.get_fee_info(luno_currency_pair)
        # Set trading fee as the max between the maker_fee and taker_fee
        trading_fee = max(float(trading_fee_response['maker_fee']),
                          float(trading_fee_response['taker_fee']))
        ...
        exchange_rate = float(ticker['last_trade'])
        to_amount = (from_amount_minus_trading_fee * exchange_rate) - luno_withdrawal_fee_ngn
